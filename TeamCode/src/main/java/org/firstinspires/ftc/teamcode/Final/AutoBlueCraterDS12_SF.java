...
public class AutoBlueCraterDS12_SF extends BaseAuto {
    ...
    private double DSPosZeroX = -59;
    // The X value of the robot position before doing DS in Position 0

    private double DSPosOneX = -48.5;
    // The X value of the robot position before doing DS in Position 1

    private double DSPosTwoX = -35.5;
    // The X value of the robot position before doing DS in Position 2
    ...
    public void loop(){
        ...
        switch(TFOD_result) {
            case 0: // Add Back Up
                // Calculating the route to the depot. (Not doing DS on this position)
                if (vumarkIsVisible()) {
                    Log.i("Vumark_Visibility", "True");
                    refreshVumarkReading();
                    currPosition = getPositionFromVumark();
            ...
            case 1: // Add Back Up
                if(vumarkIsVisible()) {
                    ...
                    Log.i("Calculated XOffset", "" + XOffset);
                    Log.i("Calculated YOffset", "" + YOffset);
                    if (Math.abs(XOffset + 40) > 15)
                    {
                        Log.i("XOffset_Overwrite", "True");
                        XOffset = -40;
                    }
                    if (Math.abs(YOffset - 14) > 7)
                    {
                        Log.i("YOffset_Overwrite", "True");
                        XOffset = 14;
                    }
