...
public class BaseOpMode extends OpMode {
    ...
    protected void setAllDrivePowerSlow(double dir,double x,double w){
        ...
        try{sleep(0,(int)(60000*(1-highp)));} catch(InterruptedException e){telemetry.addLine("Error0");}
        setAllDrivePower(-.2*dir-.5*x+.2*w,-.2*dir+.5*x+.2*w,.2*dir-.5*x+.2*w,.2*dir+.5*x+.2*w);
        try{sleep(0,(int)(60000*highp));} catch(InterruptedException e){telemetry.addLine("Error1");}
