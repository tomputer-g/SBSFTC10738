...
@TeleOp(group = "Final")
public class DEPOT_REV_3steps extends BaseTeleOp {
    ...
    private static double hatch_close = 0, hatch_open = 1;
    ...
    public void loop() {
        ...
        if(this.gamepad1.right_trigger>0.5){
            ...
            if(grabber_shoulder.getCurrentPosition()>2900) {
                ...
                if((limitSWCalibrated ? grabber_slide.getCurrentPosition() : grabber_slide.getCurrentPosition() + enc_slide_offset) < 250){
                    isMovingTo = 1;
                }else{
                    isMovingTo = 2;
                }
    ...
    private void eazy_dump(){
        switch(isMovingTo){
            ...
            case 0://back to ground (unchanged from before)
                ...
                if(near(grabber_shoulder.getCurrentPosition(),locations[0][0],150) && near(grabber_slide.getCurrentPosition(),locations[0][1],50))
                    isMovingTo = -1;
                break;
            case 1://retract slide
                ...
                if(near(grabber_slide.getCurrentPosition(),locations[1][1],70)) {
                    isMovingTo = -1;
                    grabber_shoulder.setPower(0);
                }
                break;
            case 2://dump it
                ...
                if(near(grabber_shoulder.getCurrentPosition(),locations[2][0],150) && near(grabber_slide.getCurrentPosition(),locations[2][1],70)) {
                    isMovingTo = -1;
                    grabber_shoulder.setPower(0);
                    hatch.setPosition(hatch_open);
                }
